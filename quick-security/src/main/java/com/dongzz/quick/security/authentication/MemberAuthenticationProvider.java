...
public class MemberAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
    ...
    public static final Logger logger = LoggerFactory.getLogger(MemberAuthenticationProvider.class);
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        logger.debug("Member login，username：{}", authentication.getName());
    ...
    protected void verificationCodeAuthenticationChecks(UsernamePasswordAuthenticationToken authentication) {
        ...
        if (StringUtils.isNotBlank(code)) {
            ...
            if (StringUtils.isBlank(cacheCode)) {
                throw new VerificationException(HttpStatus.UNAUTHORIZED, "验证码不存在或已经过期");
            }
     ...
     * 校验前台请求过来的 Authentication 当前仅处理 MemberAuthenticationToken
     */
    @Override
